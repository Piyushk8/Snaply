...
const PeopleQueryPage = ({ query }: { query: string }) => {
  ...
  if (!!!users?.length && status === "success")
    return (
      <div className=" space-y-3 w-full font-bold text-gray-600 text-center  bg-card p-2 rounded-2xl shadow-lg">
  ...
  if (status === "pending") {
    return (
      <div className=" space-y-3 flex w-full font-bold text-gray-600 justify-center  bg-card p-2 rounded-2xl shadow-lg">
  ...
  if (status === "error") {
    return (
      <p className="absolute z-50 space-y-3 w-full font-bold text-gray-600 text-center  bg-white p-2 rounded-2xl shadow-lg">
  ...
  return (
    <InfinityScrollContainer
      className="space-y-3 w-full p-2 bg-card  flex flex-col scrollbar-thin   overflow-y-auto max-h-screen border border-border rounded-2xl shadow-lg"
      onBottomReached={() => {
...
export const UserComponent = ({ user }: { user: userData }) => {
  ...
  return (
    <div
      ...
      className="flex items-center p-3 gap-3 bg-card hover:bg-secondary rounded-md w-full h-fit transition-shadow shadow-sm hover:shadow-md"
    >
      <Link
      ...
      >
        ...
        <div className="flex-shrink-0 w-12 h-12 rounded-full overflow-hidden bg-gray-200">
          <ClientCldImage src={user?.image} alt={`${user?.name}'s profile`} />
        </div>
