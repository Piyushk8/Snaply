...
export function useMediaUpload() {
  ...
  const [attachmentUploadInfo, setAttachmentUploadInfo] = useState<UploadingFile[]>([])
  const [allUploadComplete, setallUploadComplete] = useState(false)

  const allUploadCompleteCheck = useCallback(()=>{
    const result = attachmentUploadInfo.every((file)=>file.status==="completed"||file.status==="failed")
    if(result) {
  ...
  const uploadAttachments = async (files: File[]) => {
    ...
    const uploadPromises = renamedFiles.map(async (file) => {
      const attachment =  attachmentUploadInfo.filter((a)=>a.file.name===file.name)[0]
      ...
      try {
        ...
        const response = await uploadPromise as { url: string; publicId: string }
  ...
  const removeAttachment =async (fileName:string) => {
    setAttachmentUploadInfo(prev => prev.filter(f => f.file.name !== fileName))
    const publicId = attachmentUploadInfo.filter((a)=>a.file.name===fileName)[0].publicId
    if(publicId){
      const response = await fetch(`/api/upload-video?Id=${publicId}`,{method:"DELETE"})
    }
